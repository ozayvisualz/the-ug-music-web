  ...
  getUsers: adminProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const [users, total] = await Promise.all([
        ctx.db.user.findMany({ where, select: { id: true, name: true, email: true, phone: true, role: true, createdAt: true }, orderBy: { createdAt: "desc" }, take: input.limit, skip: input.offset }),
