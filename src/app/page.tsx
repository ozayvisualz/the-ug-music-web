...
import { getDisplayArtist } from "@/lib/utils";
import { Sparkles, Flame, Mic2, Radio } from "lucide-react";
...
export default function HomePage() {
  ...
  const handleStartListening = () => {
    if (trending?.length) {
      const queue = trending.map((s: any) => ({
        id: s.id,
        title: s.title,
        artist: getDisplayArtist(s),
          ...
          <div className="px-4 py-4 space-y-4">
            ...
            <div className="relative rounded-2xl overflow-hidden bg-gradient-to-r from-yellow-500/20 via-yellow-500/5 to-zinc-900 p-5 md:p-8">
              <div className="relative z-10">
                ...
                <div className="flex gap-3 mt-4">
                  <button onClick={handleStartListening} className="px-5 py-2.5 rounded-full bg-yellow-500 text-black font-semibold hover:bg-yellow-400 transition text-sm">Start Listening</button>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                { label: "New Songs", icon: <Sparkles className="w-5 h-5" />, href: "/discover", color: "bg-emerald-500/10 text-emerald-400" },
                { label: "Artists", icon: <Mic2 className="w-5 h-5" />, href: "/search?q=Artist", color: "bg-blue-500/10 text-blue-400" },
                { label: "Radio", icon: <Radio className="w-5 h-5" />, href: "/radio", color: "bg-purple-500/10 text-purple-400" },
              ].map((item) => (
            ...
            <section>
              <div className="flex items-center justify-between mb-3">
                ...
                <Link href="/discover" className="text-xs text-yellow-500 hover:text-yellow-400">See All</Link>
              </div>
            ...
            <section>
              <div className="flex items-center justify-between mb-3">
                ...
                <Link href="/search?q=Artist" className="text-xs text-yellow-500 hover:text-yellow-400">See All</Link>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
