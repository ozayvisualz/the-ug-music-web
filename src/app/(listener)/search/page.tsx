              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {artists.map((a: any) => (
                  <Link key={a.id} href={`/artist/${a.id}`} className="flex items-center gap-3 p-3 bg-zinc-900 border border-zinc-800 rounded-xl hover:border-yellow-500/30 transition">
                    <div className="w-10 h-10 rounded-full bg-yellow-500/10 flex items-center justify-center text-sm font-bold text-yellow-500">{a.user?.name?.charAt(0) || "?"}</div>
                    <div><p className="text-sm font-semibold">{a.user?.name}</p><p className="text-xs text-zinc-500">Artist</p></div>
                  </Link>
                ))}
              </div>
            </section>
              ...
              <div className="space-y-1">
                {songs.map((s: any) => (
                  <Link key={s.id} href={`/song/${s.id}`} className="flex items-center gap-4 p-3 hover:bg-zinc-800/50 rounded-xl transition group">
                    ...
                    <div className="flex-1 min-w-0"><p className="text-sm font-semibold truncate">{s.title}</p><p className="text-xs text-zinc-500 truncate">{s.artist?.user?.name || "Unknown"}</p></div>
